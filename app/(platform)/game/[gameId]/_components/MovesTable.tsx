      ...
      >
        {movesArray.map((movePairs, index) => {
          return (
            <div
              key={index}
              className={`w-full py-px px-4 font-bold items-stretch ${
                index % 2 !== 0 ? "bg-[#2B2927]" : ""
              }`}
            >
              <div className="grid grid-cols-6 gap-16 w-4/5">
                <span className="text-[#C3C3C0] px-2 py-1.5">{`${
                  index + 1
                }.`}</span>
                ...
                {movePairs.map((move, movePairIndex) => {
                  ...
                  return (
                    <div
                      key={movePairIndex}
                      className={`col-span-2 cursor-pointer flex items-center w-full pl-1 ${
                        isHighlighted
                          ? "bg-[#484644] rounded border-b-[#5A5858] border-b-[3px]"
                          : ""
                      }`}
                      onClick={() => {
